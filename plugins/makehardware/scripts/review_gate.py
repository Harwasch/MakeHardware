...
VIEWABLE = {".md", ".markdown", ".png", ".jpg", ".jpeg", ".gif", ".svg",
            ".pdf", ".txt", ".csv", ".json", ".yaml", ".yml", ".html"}
...
NEEDS_AN_APP = {".kicad_sch", ".kicad_pcb", ".kicad_pro", ".step", ".stp",
                ".stl", ".f3d", ".sldprt", ".drawio", ".dxf", ".gbr", ".zip",
                ".asc", ".cir", ".raw", ".sdoc"}
...
def write_packet(review: dict) -> str:
    ...
    if st == "approved":
        o += [f'**Approved** {review.get("decided", "")}'
              f' by {review.get("reviewer", "the human")}.', ""]
        if review.get("note"):
            o += ["> " + review["note"].strip().replace("\n", "\n> "), ""]
    elif st == "changes_requested":
        o += [f'**Changes requested** {review.get("decided", "")}.', ""]
        if review.get("note"):
            o += ["> " + review["note"].strip().replace("\n", "\n> "), ""]
    elif st == "stale":
        o += ["**Stale.** This was approved, then an artefact changed:", ""]
        o += [f"* `{p}`" for p in drifted(review)]
        o += ["", "It needs looking at again before the stage counts as done.", ""]
    else:
        o += ["Not yet reviewed. Recorded with:", "",
              "```bash",
